...
def get_price(coin, curr=None):
    ...
    try:
        ...
        return [(data_raw[c][curr]['PRICE'],
                ...
                data_raw[c][curr]['LOW24HOUR'], data_raw[c][curr]['MKTCAP']) for c in coin.split(',') if c in data_raw.keys()]
    except:
...
def marketCapFormatter(val):
    firstThreeDigits = int(str(val)[:3])    
    if (val / 1000000000000) >= 1:
        return (" $" + str(firstThreeDigits) + "T")
    elif (val / 1000000000) >= 1:
        return (" $" + str(firstThreeDigits) + "B")
    elif (val / 1000000) >= 1:
        return (" $" + str(firstThreeDigits) + "M")
    else:
        return val
...
def str_formatter(coin, val, held):
    ...
    return '  {:<5} {:>{}}  {} {:>{}} {:>{}} {:>{}} {:>{}} '.format(coin,
        ...
        marketCapFormatter(val[3]), avg_length)
...
def write_scr(stdscr, wallet, y, x):
    ...
    if y >= 2:
        header = '  COIN{}PRICE{}HELD {}VAL{}HIGH {}LOW {}MCAP'.format(first_pad, second_pad, third_pad, first_pad, first_pad, first_pad)
    ...
    if coinl:
        ...
        if y > 3:
            ...
            coinl = list(x[0] for x in s) #"BTC"
            coinvl = list(x[1] for x in s) #"[price, high, low, mktcap"
